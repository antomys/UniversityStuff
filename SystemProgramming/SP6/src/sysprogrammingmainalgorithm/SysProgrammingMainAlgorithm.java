...
public class SysProgrammingMainAlgorithm {
    public static void main(String[] args) throws IOException {
        ...
        boolean result;
        String filename;
        MyLang testLang = new MyLang("Examples/Pascal/PASCAL_GR.TXT", 1);
        ...
        System.out.println("Введіть шлях до Паскаль програми: ");
        String path;
        try(Scanner in = new Scanner(System.in)){
            path=in.nextLine();
            TestPascalParser pascalParser = new TestPascalParser(testLang, path);
            pascalParser.parserRecursive2();
        }
    ...
    MyLangMine myLangMine=null;
    int codeAction, llk=1, textLen;
    String [] menu= { "*1.  Прочитати граматику з файла  ",
            " 2.  Лабораторна робота. Клас будує студент(4.\tПеревірити, чи  L(G) – пуста множина)",
        ...
        switch (codeAction) {
            case 1: //1. Прочитати граматику з файла",
                ...
                try {
                    ...
                    myLangMine=new MyLangMine(fileName,llk);
                }
                catch(Exception ee)
                { System.out.println ("Системна помилка: "+ee.toString());
                    return;
            ...
            case 2: //2. Лабораторна робота студента
                myLangMine.FirstKFollowAi();;
                myLangMine.GrammarIsLL1();
                break;
            ...
            case 22: // PASCAL
                String path;
                System.out.println("Enter the path to the Pascal program file:\n");
                try (Scanner in = new Scanner(System.in)) {
                    path = in.nextLine();
                    Pascal parser = new Pascal(testLang,path);
                    parser.parse();
                }
                catch (IOException ex){
                    ex.printStackTrace();
